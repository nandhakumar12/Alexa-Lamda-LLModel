...
class VoiceAssistantAI:
    ...
    def try_llm_response(self, message: str, user_id: str) -> Optional[str]:
        """Try to get response from LLM Lambda function"""
        try:
...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if not message:
            ...
            # Try LLM first, fallback to enhanced chatbot
            response_text = assistant.try_llm_response(message, user_id)
            if not response_text:
                # Fallback to enhanced chatbot
