#!/usr/bin/env python3
"""
Simple Frontend Deployment for Voice Assistant AI
Uses npx instead of npm for package management
"""
...
class SimpleFrontendDeployer:
    ...
    def install_dependencies(self) -> bool:
        """Install frontend dependencies using npx"""
        print("📦 Installing frontend dependencies...")
        
        try:
            os.chdir('frontend')
            
            # Use npx to install dependencies
            result = subprocess.run(['npx', 'npm', 'install'], capture_output=True, text=True)
            if result.returncode == 0:
                print("✅ Dependencies installed successfully")
                return True
            else:
                print(f"⚠️  npm install failed, trying alternative approach...")
                # Try using npx directly with create-react-app
                result2 = subprocess.run(['npx', 'create-react-app', '--version'], capture_output=True, text=True)
                if result2.returncode == 0:
                    print("✅ npx create-react-app available")
                    return True
                else:
    ...
    def build_frontend(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(['npx', 'npm', 'run', 'build'], env=env, capture_output=True, text=True)
            if result.returncode == 0:
                print("✅ Frontend built successfully")
                return True
            else:
                print(f"⚠️  npm run build failed, trying alternative...")
                # Try direct build command
                result2 = subprocess.run(['npx', 'react-scripts', 'build'], env=env, capture_output=True, text=True)
                if result2.returncode == 0:
                    print("✅ Frontend built successfully with react-scripts")
                    return True
                else:
                    print(f"❌ Build failed: {result2.stderr}")
                    return False
        except Exception as e:
...
def main():
    """Main function"""
    deployer = SimpleFrontendDeployer()
