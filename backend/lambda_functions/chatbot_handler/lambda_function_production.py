...
class VoiceAssistantAI:
    ...
    def call_llm_function(self, message: str, user_id: str) -> Optional[str]:
        """Call the LLM Lambda function for advanced responses"""
        try:
            payload = {
                'body': json.dumps({
                    'message': message,
    ...
    def process_smart_query(self, message: str, user_id: str) -> str:
        ...
        llm_response = self.call_llm_function(message, user_id)
        if llm_response:
            return llm_response
...
Just ask me anything naturally, like "What's the weather in London?" or "What time is it?"
        ...
        # Default intelligent response (LLM unavailable)
        return f"""I understand you said: "{message}"

I'm your AI assistant! My advanced LLM is temporarily unavailable, but I can still help with:
• Weather: "What's the weather like?"
• Time: "What time is it?"
• Features: "What can you do?"
• Basic conversations

Try toggling 'LLM Mode' in the interface for enhanced conversations, or ask me something specific!"""
